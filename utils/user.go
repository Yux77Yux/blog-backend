...
func SignIn(user model.UsernameAndPassword) (model.UserActive, error) {
	db, err := sql.Open("mysql", config.ConnectionStr)
	if err != nil {
		return model.UserActive{}, err
	}
	defer db.Close()
	...
	WHERE user.username = ? 
	AND user.password = ? 
	AND user_incidental.status = 0
	`

	var currentUser model.UserActive
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return model.UserActive{}, fmt.Errorf("user not found")
		}
		return model.UserActive{}, err
	}
